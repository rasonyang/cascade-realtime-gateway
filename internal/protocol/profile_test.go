...
func TestProfileRejectedFields(t *testing.T) {
	cases := []struct {
		...
		{"tool not an object", `{"type":"session.update","session":{"type":"realtime","tools":["f"]}}`, "invalid_value", "session.tools[0]"},
		{"tool type missing", `{"type":"session.update","session":{"type":"realtime","tools":[{"name":"f"}]}}`, "invalid_value", "session.tools[0].type"},
		{"mcp tool", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"mcp","server_label":"s"}]}}`, "invalid_value", "session.tools[0].type"},
		{"tool name missing", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"function"}]}}`, "invalid_value", "session.tools[0].name"},
		{"duplicate tool name", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"function","name":"f"},{"type":"function","name":"f"}]}}`, "invalid_value", "session.tools[1].name"},
		{"tool parameters not an object", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"function","name":"f","parameters":[1]}]}}`, "invalid_value", "session.tools[0].parameters"},
		{"tool unknown field", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"function","name":"f","strict":true}]}}`, "invalid_value", "session.tools[0].strict"},
		{"tool_choice unknown mode", `{"type":"session.update","session":{"type":"realtime","tool_choice":"sometimes"}}`, "invalid_value", "session.tool_choice"},
		{"tool_choice mcp", `{"type":"session.update","session":{"type":"realtime","tool_choice":{"type":"mcp","server_label":"s"}}}`, "invalid_value", "session.tool_choice.type"},
		{"tool_choice forced unknown", `{"type":"session.update","session":{"type":"realtime","tools":[{"type":"function","name":"f"}],"tool_choice":{"type":"function","name":"g"}}}`, "invalid_value", "session.tool_choice.name"},
		{"tool_choice bad json", `{"type":"session.update","session":{"type":"realtime","tool_choice":7}}`, "invalid_value", "session.tool_choice"},
		{"client function_call item", `{"type":"conversation.item.create","item":{"type":"function_call","name":"f","call_id":"call_1","arguments":"{}"}}`, "invalid_value", "item.type"},
		{"tool output unknown call", `{"type":"conversation.item.create","item":{"type":"function_call_output","call_id":"call_nope","output":"ok"}}`, "invalid_value", "item.call_id"},
		{"tool output missing call_id", `{"type":"conversation.item.create","item":{"type":"function_call_output","output":"ok"}}`, "invalid_value", "item.call_id"},
		{"tool output missing output", `{"type":"conversation.item.create","item":{"type":"function_call_output","call_id":"call_1"}}`, "invalid_value", "item.output"},
		{"tool output with role", `{"type":"conversation.item.create","item":{"type":"function_call_output","call_id":"call_1","output":"ok","role":"tool"}}`, "invalid_value", "item.role"},
...
func TestProfileIgnoredFieldsEcho(t *testing.T) {
	...
	checks := map[string]any{
		"session.model":                                         "m2",
		"session.tool_choice":                                   "none", // effective now, no longer echoed-only
